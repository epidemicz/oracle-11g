...
change_dpdump_dir () {
	echo_green "Changing dpdump dir to /opt/oracle/dpdump"
	sqlplus / as sysdba <<-EOF |
...
run_db_setup_scripts () {
	echo_green "Running db-setup-scripts"
	echo "$0: running /db-setup-scripts/01-dbsetup.sql"; echo "exit";
	$ORACLE_HOME/bin/sqlplus / as sysdba @/db-setup-scripts/01-dbsetup.sql
	echo "$0: running /db-setup-scripts/02-import-dump.sh"; /db-setup-scripts/02-import-dump.sh
}
